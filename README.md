# Spiralverse Chronicles

Lore articles, character profiles, and world-building for the **Spiralverse** -- the narrative universe behind [SCBE-AETHERMOORE](https://aethermoorgames.com).

## What Is the Spiralverse?

A multi-dimensional fantasy universe where the most powerful magic requires **cooperation**. At its center lies **Avalon** -- a collaborative educational civilization anchored to a living, conscious World Tree that bridges realities.

The Spiralverse is also the narrative layer for the **Spiral Cascade Behavioral Engine (SCBE)** -- a 14-layer AI safety and governance framework that uses the same collaboration-vs-domination axis to train artificial intelligence systems.

## Articles

### World-Building
- [The Spiralverse: An Introduction to Aethermoor](articles/01_the_spiralverse_introduction.md) -- Overview of the world, its characters, and its philosophical core

### Character Profiles
- [Izack Thorne: The Dimensional Architect](articles/02_izack_thorne_dimensional_architect.md) -- The visionary who built Avalon and paid for it personally
- [Aria Ravencrest: The Mother of Avalon](articles/03_aria_ravencrest_mother_of_avalon.md) -- The boundary mage whose philosophy became a civilization
- [Polly: The Raven Who Remembers Everything](articles/04_polly_the_raven_who_remembers.md) -- The sentient intelligence who might be narrating the whole story

### Lore Guides
- [The Six Sacred Tongues](articles/05_six_sacred_tongues.md) -- How magic feels: emotional languages that shape the Spiralverse

### Editorials
- [Why Collaborative Magic Is Spiralverse's Strongest Edge](articles/06_collaborative_magic_strongest_edge.md) -- Market positioning: magic as social technology
- [Hyperbolic Geometry and AI Safety](articles/07_hyperbolic_geometry_ai_safety.md) -- Lore-to-governance bridge through geometric policy
- [The 14-Layer Security Pipeline](articles/08_14_layer_security_pipeline.md) -- How the SCBE stack maps into narrative and platform control
- [Post-Quantum Cryptography in SCBE](articles/09_post_quantum_cryptography_scbe.md) -- Quantum-resilient trust framing
- [Energy-Aware Compute Governor](articles/10_energy_aware_compute_governor.md) -- Scheduling and cost-control surface
- [The Honest 34 Percent Benchmark](articles/11_honest_benchmark_34_percent.md) -- What failed, what generalized, and why the gap matters
- [Trichromatic Forgery Resistance](articles/12_trichromatic_forgery_resistance.md) -- Hidden-band structural detection notes

## Corpus and Training Data

- `corpus/manuscripts/avalon-codex-complete-unified.txt` -- long-form Avalon codex/manuscript body
- `corpus/chapters/revised-chapters-with-ancient-traditions.txt` -- chapter packet with revised scene prose
- `corpus/character-sheets/avalon-codex-character-sheet.txt` -- compact cast and artifact sheet
- `training-data/lore_sessions/*.jsonl` -- JSONL lore/training records for characters, canon, roleplay sessions, and deep world context

## Provenance

- The current corpus import came from a stable local restart backup under `Dropbox (Old)`, not a live sync root.
- Exact source paths and duplicate-variant notes live in [manifests/import-2026-03-30-dropbox-old.md](manifests/import-2026-03-30-dropbox-old.md).

## Core Characters

| Character | Role | Tongue Affinity |
|-----------|------|----------------|
| **Izack Thorne** | Dimensional architect, Avalon founder | -- |
| **Aria Ravencrest** | Boundary mage, co-founder, Mother of Avalon | -- |
| **Polly** | Sentient raven, co-equal guide, archivist | -- |
| **Alexander Thorne** | Prodigy, realm consciousness bridge | -- |
| **Zara Millwright** | First student, senior leader | -- |
| **Malzeth'irun** | Demon-realm ambassador | -- |
| **Clay** | Loyal protector | RU |
| **Eldrin** | Dimensional scholar, mentor | AV |

## The Six Sacred Tongues

| Tongue | Code | Emotion | Society |
|--------|------|---------|---------|
| Kor'aelin | KO | Collaborative love | Weavebinders |
| Avali | AV | Hopeful openness | Resonance Guild |
| Ru'khari | RU | Ancestral memory | Deep Archive Keepers |
| Cael'thara | CA | Playful joy | Sparkwright Collective |
| Umbriel | UM | Honest melancholy | Threshold Mourners |
| Draumric | DR | Fierce pride | Oath Forged |

## Related Projects

- [SCBE-AETHERMOORE](https://aethermoorgames.com) -- The AI safety framework
- [Six Tongues GeoSeal](https://github.com/issdandavis/six-tongues-geoseal) -- Cryptographic toolkit

---

*Created by Issdandavis. Patent pending: USPTO #63/961,403.*
